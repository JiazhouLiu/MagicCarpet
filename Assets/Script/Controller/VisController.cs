...
public class VisController : MonoBehaviour
{
    ...
    private List<Transform> CombineFeetVisAndAssignPosition(List<Transform> leftVis, List<Transform> rightVis)
    {
        ...
        {
            HeadDashboard.GetComponent<DashBoard>().ForwardParameter = 4;
            if (currentVisOnDashboard.Count > 0)
            {
                // remove old vis
        ...
        else
        {
            ...
            HeadDashboard.GetComponent<DashBoard>().ForwardParameter = 8;
    ...
    //                    foreach (Transform t in showOnDashboard)
    //                        VisDistanceToHuman.Add((1 / Vector3.Distance(t.position, CameraTransform.position)));
    ...
    {
        float humanToTarget = Vector3.Distance(CameraTransform.position, targetVis.position);
        float leftEdgeLength = Vector3.Distance(targetVis.position, prevVis.position);
        float rightEdgeLength = Vector3.Distance(targetVis.position, nextVis.position);
    ...
    private List<Transform> RearrangeDisplayBasedOnAngle(List<Transform> markers)
    {
        ...
        if (markers != null && markers.Count > 0)
        {
            ...
            foreach (Transform t in markers)
                markerAnglesToHuman.Add(t, Vector3.SignedAngle(CameraTransform.forward, t.position - CameraTransform.position, Vector3.up));
    ...
    {
        Vector3 Human2DPosition = new Vector3(CameraTransform.position.x, 0, CameraTransform.position.z);
    ...
    private Dictionary<string, Transform> CheckHumanInTriangles()
    {
        ...
        foreach (HalfEdgeFace2 face in CurrentTriangles.faces)
        {
            ...
            if (PointInTriangle(CameraTransform.position, a, b, c))
            {
                foreach (Transform t in GroundVisParent)
                {
    ...
    private void PinToGround(Transform t)
    {
        ...
        visOnGround.transform.position = new Vector3(CameraTransform.position.x, 0, CameraTransform.position.z);
